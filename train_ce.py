...
def train_one_epoch(encoder, trainLoader, optimizer, loss_fn, args):
    ...
    for idx, batch in enumerate(trainLoader):
        ...
        if (idx + 1) % 20 == 0:
            ...
            print('  batch {} loss: {}, {}'.format(idx + 1, loss_ce.item(), last_loss))
            running_loss = 0.
            print("  torch.unique(gt) : ", torch.unique(gt))
    ...
    return avg_total_running_loss_ce
...
def main(args):
    ...
    encoder = Encoder(clip_model, unique_part_names, args.score_map_type)
    ...
    for epoch in range(args.starting_epoch - 1, args.epochs):
        ...
        encoder.train()
        avg_loss_ce = train_one_epoch(encoder, trainLoader, optimizer,loss_fn, args)
        
        # We don't need gradients on to do reporting
        
        encoder.eval()
        with torch.no_grad():
            avg_vloss_ce  = validation(encoder, testLoader, loss_fn, args)
        avg_vloss = avg_vloss_ce
        ...
        if avg_vloss < best_vloss:
            ...
            model_path = os.path.join(args.model_dir, 'best_model_ce_simple_decoder')
            if args.multi_step_scheduler:
        ...
        model_path = os.path.join(args.model_dir, 'last_model_ce_simple_decoder')
        if args.multi_step_scheduler:
            torch.save({'state_dict': encoder.state_dict(),
                       'optimizer': optimizer.state_dict(),
                       'scheduler': scheduler.state_dict(),
